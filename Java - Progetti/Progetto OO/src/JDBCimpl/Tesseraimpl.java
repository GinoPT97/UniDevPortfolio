	...
	public ArrayList<Tessera> alltessera() throws SQLException {
		...
		while (rs.next()) {
			tess.add(new Tessera(rs.getString("codtessera"), rs.getInt("numeropunti"),
					new Cliente(null, rs.getString("nome"), rs.getString("cognome"), rs.getString("codicefiscale"),
							rs.getString("email"), rs.getString("indirizzo"), rs.getString("telefono"), null, null)));
		}
		rs.close();
		return tess;
	}
