...
Questa sezione approfondisce le modalità di autenticazione e i permessi differenziati tra amministratori e dipendenti, illustrando i flussi di accesso e le funzionalità disponibili per ciascun ruolo.

\subsection*{Login e Autenticazione}
All’avvio, l’utente accede tramite una schermata di login che richiede credenziali personali. Il sistema verifica le credenziali e, in base al ruolo (amministratore o dipendente), abilita le relative funzionalità. Sono gestiti i casi di errore (credenziali errate, utente non trovato) e la sicurezza delle password. L’interfaccia è progettata per essere intuitiva e sicura.

\subsection*{Funzionalità Admin}
L’amministratore ha accesso a tutte le funzionalità del sistema: può gestire l’anagrafica di dipendenti e clienti, inserire, modificare o eliminare prodotti, consultare tutte le statistiche e i report, e visualizzare le classifiche di vendita. Può inoltre gestire i permessi degli utenti e monitorare l’attività del negozio tramite dashboard dedicate.

\subsection*{Funzionalità Dipendente}
Il dipendente può gestire gli ordini, consultare e modificare i dati dei clienti, visualizzare i prodotti e il proprio storico vendite. Ha accesso a statistiche personali e può ricercare clienti o prodotti per facilitare le operazioni di vendita. Le sue azioni sono tracciate per garantire trasparenza e responsabilità.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.35\textwidth,keepaspectratio]{../DocImm/CD.png}
    \caption{Diagramma delle classi (Class Diagram)}
\end{figure}
\begin{center}
    \small{\href{#fig:cd-zoom}{Clicca qui per ingrandire}}
\end{center}

% --- Appendice: versione ingrandita ---
\clearpage
\phantomsection
\addcontentsline{toc}{subsection}{Diagramma delle classi (zoom)}
\label{fig:cd-zoom}
\begin{figure}[p]
    \centering
    \includegraphics[width=1.0\textwidth]{../DocImm/CD.png}
    \caption{Diagramma delle classi (versione ingrandita)}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth,keepaspectratio]{../DocImm/CreaOrdine-SD.png}
    \caption{Diagramma di sequenza: Creazione Ordine}
\end{figure}
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{../DocImm/NuovoCliente-SD.png}
