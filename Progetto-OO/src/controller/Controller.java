...
public class Controller {
	...
	private static final String[] CLIENTE_COLUMNS = { "Id Cliente", "Nome", "Cognome", "Codice fiscale", "Email",
			"Indirizzo", "Telefono", "Id Tessera", "Punti", "Stato Tessera", "Scadenza" };
	private static final String[] DIPENDENTE_COLUMNS = { "Id", "Nome", "Cognome", "Codice fiscale", "Email",
			"Indirizzo", "Telefono" };
	private static final String[] PRODOTTO_COLUMNS = { "Id", "Nome", "Descrizione", "Prezzo", "Provenienza", "Raccolta",
			"Mungitura", "Glutine", "Scadenza", "Produzione", "Categoria", "Scorta" };
	...
	public void returnToLastFrame() {
		setVisibleFrame(lastFrame, searchf, visordf, vprodf, vdipf, statdipf, visctf, carrf, nprodf, ndipf, nclf,
				modprodf, upclf, updipf);
	}
...
// Blocco vuoto intenzionale: nessuna azione richiesta per il valore di default
	...
	public boolean newdip(String codDipendente, String nome, String cognome, String codFis, String email,
			String indirizzo, String telefono) throws SQLException {
		...
		// Se l'inserimento nel database ha successo, aggiorna anche il model della
		// tabella
		if (success)
	...
	public boolean newclt(String codCliente, String nome, String cognome, String codFis, String email, String indirizzo,
	...
	public boolean nuovatessera(String codFisc) throws SQLException {
		tessera = new Tessera("", 0.0, java.time.LocalDate.now(), java.time.LocalDate.now().plusYears(1), "ATTIVA",
				cliente);
	...
	public boolean newprod(String codProdotto, String nome, String descrizione, double prezzo, String luogoProvenienza,
			java.time.LocalDate dataRaccolta, java.time.LocalDate dataMungitura, boolean glutine,
			java.time.LocalDate dataScadenza, java.time.LocalDate dataproduzione, String categoria, int scorta)
			throws SQLException {
		prodotto = new Prodotto(codProdotto, nome, descrizione, prezzo, luogoProvenienza, dataRaccolta, dataMungitura,
				glutine, dataScadenza, categoria, scorta, dataproduzione);
		boolean success = prdjdbc.setNewProdotto(prodotto);
		// Se l'inserimento nel database ha successo, aggiorna anche il model della
		// tabella
		if (success)
			updateProdottoModelAfterInsert(codProdotto, nome, descrizione, prezzo, luogoProvenienza, dataRaccolta,
	...
	public boolean upprod(String codProdotto, String nome, String descrizione, double prezzo, String luogoProvenienza,
			java.time.LocalDate dataRaccolta, java.time.LocalDate dataMungitura, boolean glutine,
			java.time.LocalDate dataScadenza, String categoria, int scorta) throws SQLException {
		Prodotto pe = new Prodotto(codProdotto, nome, descrizione, prezzo, luogoProvenienza, dataRaccolta,
				dataMungitura, glutine, dataScadenza, categoria, scorta, null);
		boolean success = prdjdbc.updateProdotto(pe);
		if (success)
			updateProdottoModelAfterUpdate(codProdotto, nome, descrizione, prezzo, luogoProvenienza, dataRaccolta,
					dataMungitura, glutine, dataScadenza, categoria, scorta);
		return success;
	}

	// Aggiorna le informazioni di un dipendente
	public boolean updip(String codDipendente, String nome, String cognome, String codFis, String email,
			String indirizzo, String telefono) throws SQLException {
		boolean success = dpjdbc
				.updatedipendente(new Dipendente(codDipendente, nome, cognome, codFis, email, indirizzo, telefono));
		if (success)
	...
	public boolean upcliente(String codCliente, String nome, String cognome, String codFis, String email,
			String indirizzo, String telefono) throws SQLException {
		boolean success = cljdbc
				.updateCliente(new Cliente(codCliente, nome, cognome, codFis, email, indirizzo, telefono, null, null));
		if (success)
	...
	public boolean nuovoordine(String codOrdine, Date dataAcquisto, double prezzoTotale, int idCliente,
			int idDipendente) throws SQLException {
	...
	public boolean newarticoli(String codOrdine, String codProdotto, double prezzo, int numeroArticoli)
			throws SQLException {
		articoli = new Articoli(codOrdine, codProdotto, prezzo, numeroArticoli, 0); // codCliente non necessario per
																					// articoliordine
	...
	public void ClientSearch(DefaultTableModel model) throws SQLException {
		...
		populateTable(clienti, model, c -> createClientSearchRowData(c.getCodCliente(), // Cod Cliente
				c.getNome(), // Nome
	...
	private Object[] createClientSearchRowData(Object codCliente, Object nome, Object cognome, Object categoria,
			Object puntiCategoria, Object spesaTotale, Object ordiniCategoria) {
		return new Object[] { checkNull(codCliente), // Colonna 0: Cod Cliente (String)
	...
	public void categoriaprodotti(String c, DefaultTableModel model) throws SQLException {
		...
		populateTable(prodotti, model,
				p -> createRowData(p.getCodProdotto(), p.getNome(), p.getPrezzo(), p.getCategoria(), p.getScorta()));
	}

	public void allCliente() throws SQLException {
		List<Cliente> clienti = cljdbc.getAllCt();
		populateTable(clienti, clienteModel,
				c -> createRowData(c.getCodCliente(), c.getNome(), c.getCognome(), c.getCodiceFiscale(), c.getEmail(),
						c.getIndirizzo(), c.getTelefono(),
						...
						c.getTessera() != null && c.getTessera().getDataScadenza() != null
								? c.getTessera().getDataScadenza().toString()
								: null));
	}

	public void allDipendenti() throws SQLException {
		List<Dipendente> dipendenti = dpjdbc.getAllDip();
		populateTable(dipendenti, dipModel, d -> createRowData(d.getCodDIP(), d.getNome(), d.getCognome(),
				d.getCodFis(), d.getEmail(), d.getInd(), d.getTel()));
	}

	public void allOrdini() throws SQLException {
		List<Cliente> clienti = cljdbc.getAllCt();
		List<Dipendente> dipendenti = dpjdbc.getAllDip();
		Map<String, Cliente> clientiMap = clienti.stream()
				.collect(Collectors.toMap(c -> String.valueOf(c.getCodCliente()), c -> c));
		Map<String, Dipendente> dipMap = dipendenti.stream()
				.collect(Collectors.toMap(d -> String.valueOf(d.getCodDIP()), d -> d));
		...
		populateTable(ordini, ordModel, o -> {
			...
			return createRowData(o.getCodOrdine(), o.getDataAcquisto(), o.getPrezzoTotale(), clienteNome,
					dipendenteNome);
		});
	}
	...
	public void allProdotti() throws SQLException {
		...
		populateTable(prodotti, prodModel,
				p -> createRowData(p.getCodProdotto(), p.getNome(), p.getDescrizione(), p.getPrezzo(),
						p.getLuogoProvenienza(), p.getDataraccolta(), p.getDatamungitura(),
						formatGlutineStatus(p.isGlutine()), p.getDatascadenza(), p.getDataProduzione(),
						p.getCategoria(), p.getScorta()));
	}

	// --- Metodi di supporto per l'aggiornamento dei model delle tabelle
	// (CENTRALIZZATI E OTTIMIZZATI) ---
	private void updateDipendenteModelAfterInsert(String codDipendente, String nome, String cognome, String codFis,
			String email, String indirizzo, String telefono) {
		dipModel.addRow(createRowData(codDipendente, nome, cognome, codFis, email, indirizzo, telefono));
	}

	private void updateClienteModelAfterInsert(String codCliente, String nome, String cognome, String codFis,
			String email, String indirizzo, String telefono) {
		// Inserisce valori di default per la tessera: punti 0.0, stato "Attiva", data
		// odierna
		clienteModel.addRow(createRowData(codCliente, nome, cognome, codFis, email, indirizzo, telefono, "", // Id
																												// Tessera
																												// (da
																												// aggiornare
																												// se
																												// disponibile)
				0.0, "ATTIVA", java.time.LocalDate.now().toString() // Data odierna
		));
	}

	private void updateProdottoModelAfterInsert(String codProdotto, String nome, String descrizione, double prezzo,
			String luogoProvenienza, java.time.LocalDate dataRaccolta, java.time.LocalDate dataMungitura,
			boolean glutine, java.time.LocalDate dataScadenza, String categoria, int scorta) {
		prodModel.addRow(createRowData(codProdotto, nome, descrizione, prezzo, luogoProvenienza, dataRaccolta,
				dataMungitura, formatGlutineStatus(glutine), dataScadenza, null, categoria, scorta));
	}

	private void updateDipendenteModelAfterUpdate(String codDipendente, String nome, String cognome, String codFis,
			String email, String indirizzo, String telefono) {
		updateTableRow(dipModel, codDipendente,
				new Object[] { codDipendente, nome, cognome, codFis, email, indirizzo, telefono });
	}

	private void updateClienteModelAfterUpdate(String codCliente, String nome, String cognome, String codFis,
			String email, String indirizzo, String telefono) {
		updateTableRow(clienteModel, codCliente,
				new Object[] { codCliente, nome, cognome, codFis, email, indirizzo, telefono });
	}

	private void updateProdottoModelAfterUpdate(String codProdotto, String nome, String descrizione, double prezzo,
			String luogoProvenienza, java.time.LocalDate dataRaccolta, java.time.LocalDate dataMungitura,
			boolean glutine, java.time.LocalDate dataScadenza, String categoria, int scorta) {
		updateTableRow(prodModel, codProdotto, new Object[] { codProdotto, nome, descrizione, prezzo, luogoProvenienza,
				dataRaccolta, dataMungitura, formatGlutineStatus(glutine), dataScadenza, null, categoria, scorta });
	}
}