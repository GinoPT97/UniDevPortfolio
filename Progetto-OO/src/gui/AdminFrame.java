...
public class AdminFrame extends JFrame {
	...
	public AdminFrame(String title, Controller c) {
		...
		JPanel titlePanel = new ImagePanel(
				new ImageIcon(ImagePanel.class.getResource("/Immagini/ImmAdmin.png")).getImage());
	...
	private void azioni(Controller c) {
		// Listener per il bottone di logout che richiama il metodo logout nel
		// Controller
		logoutbutton.addActionListener(e -> c.logout(1));

		// Listener per il bottone dipendenti che richiama il metodo adminAndElem con
		// parametro 1 per dipendenti
		dipbutton.addActionListener(e -> c.adminAndElem(1)); // Usa adminAndElem per amministratori

		// Listener per il bottone prodotti che richiama il metodo adminAndElem con
		// parametro 2
		prodbutton.addActionListener(e -> c.adminAndElem(2)); // Usa adminAndElem per amministratori

		// Listener per il bottone statistiche che richiama il metodo adminAndElem con
		// parametro 3
		statistichebutton.addActionListener(e -> c.adminAndElem(3)); // Usa adminAndElem per amministratori

		// Listener per il bottone visione ordini che richiama il metodo adminAndElem
		// con parametro 4
		visordbutt.addActionListener(e -> c.adminAndElem(4)); // Usa adminAndElem per amministratori

		// Listener per il bottone ricerca che richiama il metodo searchAndElem con
		// parametro 1
