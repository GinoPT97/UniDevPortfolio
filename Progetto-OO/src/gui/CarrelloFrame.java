...
public class CarrelloFrame extends JFrame {
	...
	private static final String[] CATEGORIE = { CATEGORIA_TUTTI, "Frutta", "Verdura", "Farinacei", "Latticini", "Uova",
			"Confezionati" };
	...
	public void popolazioni(Controller c) {
		...
		for (int i = 0; i < c.clienteModel.getRowCount(); i++)
			clienteComboBox.addItem(String.format("%s - %s %s", c.clienteModel.getValueAt(i, 0),
	...
	private boolean categoriaMatch(String categoriaSelezionata, String categoriaProdotto) {
		...
		return catSelNorm.equals(CATEGORIA_TUTTI.toLowerCase()) || catProdNorm.equals(catSelNorm)
				...
				|| (catSelNorm.equals("uova") && catProdNorm.contains("uova")) || (catSelNorm.equals("confezionati")
						&& (catProdNorm.contains("inscatolati") || catProdNorm.contains("confezionati")));
	}
