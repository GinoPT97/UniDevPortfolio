...
public class ModificaProdottiFrame extends JFrame {
	...
	public void viewprod(String codProdotto, String nome, String descrizione, String luogoProvenienza, double prezzo,
			int scorta, boolean glutine, String categoria) {
		cod = codProdotto;
		nometf.setText(nome);
	...
	private boolean validateFields() {
		...
		for (int i = 0; i < dateFields.length; i++) {
			...
			if (!text.isEmpty() && !text.matches("^\\d{4}-\\d{2}-\\d{2}$")) {
				...
				valid = false;
				if (firstError == -1)
					firstError = obbligatori.length + 1 + i;
			}
		}
		// Focus sul primo errore
		if (!valid && firstError != -1)
			if (firstError < obbligatori.length)
				obbligatori[firstError].requestFocus();
			else if (firstError == obbligatori.length)
				descta.requestFocus();
			else
				dateFields[firstError - obbligatori.length - 1].requestFocus();
		return valid;
	}
	...
	private void azioni(Controller c) {
		...
		updatebutton.addActionListener(e -> {
			...
			try {
				...
				if ((UOVA.equals(categoria) || CONFEZIONATI.equals(categoria) || LATTICINI.equals(categoria))
						&& !scadtf.getText().trim().isEmpty())
					dataScadenza = java.time.LocalDate.parse(scadtf.getText().trim(), formatter);
				c.upprod(cod, nometf.getText(), descta.getText(), Double.parseDouble(prezzotf.getText()),
						provtf.getText(), dataRaccolta, dataMungitura, glutcb.isSelected(), dataScadenza, categoria,
