...
public class DipendenteFrame extends JFrame {
	...
	public DipendenteFrame(String title, Controller c) {
		...
		JPanel titlePanel = new ImagePanel(
				new ImageIcon(ImagePanel.class.getResource("/Immagini/ImmDipendenti.jpg")).getImage());
	...
	private void azioni(Controller c) {
		// Listener per il pulsante di logout, chiama il metodo logout del Controller
		// con parametro 2
		logoututton.addActionListener(e -> c.logout(2));

		// Listener per il pulsante cliente, chiama il metodo dipAndElem del Controller
		// con parametro 1 per i dipendenti
		clientebutton.addActionListener(e -> c.dipAndElem(1));

		// Listener per il pulsante ordine, chiama il metodo dipAndElem del Controller
		// con parametro 3
		ordineutton.addActionListener(e -> c.dipAndElem(3));

		// Listener per il pulsante di ricerca, chiama il metodo dipAndElem del
		// Controller con parametro 5
		searchbutton.addActionListener(e -> c.dipAndElem(5));

		// Listener per il pulsante prodotti, chiama il metodo dipAndElem del Controller
		// con parametro 2
