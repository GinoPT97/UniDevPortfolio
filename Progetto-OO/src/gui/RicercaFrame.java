...
public class RicercaFrame extends JFrame {
	...
	private static final String[] CATEGORIE = { "FRUTTA", "VERDURA", "FARINACEI", "LATTICINI", "UOVA", "CONFEZIONATI",
			TUTTI };
	...
	private static final Object[] searchcolonne = { "Cod Cliente", "Nome", "Cognome", "Categoria", "Punti Categoria",
			"Spesa Totale", "Ordini Categoria" };
	...
	private void azioni(Controller c) throws SQLException {
		...
		aggiornaButton.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(this, "Errore durante l'aggiornamento dei clienti.", "Errore",
	...
	private void applicaFiltri() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Errore nell'applicazione dei filtri: " + ex.getMessage(), "Errore",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private RowFilter<Object, Object> creaPuntiFilter(String intervalloPunti) {
		...
		case "501-1000" -> RowFilter.andFilter(List.of(RowFilter.numberFilter(RowFilter.ComparisonType.AFTER, 500, 4),
				RowFilter.numberFilter(RowFilter.ComparisonType.BEFORE, 1001, 4)));
		case "1001-5000" -> RowFilter.andFilter(List.of(RowFilter.numberFilter(RowFilter.ComparisonType.AFTER, 1000, 4),
