...
# Codice Python per la funzione SalvaToExcel
code_salva_to_excel = """
...
# Codice Python da evidenziare
code_statistiche_giri = """
def StatisticheGiri(EventsDF, FixationsDF, BlinksDF, SaccadesDF, EyeStatesDF, cartella):
    ...
    for i in range(len(giri) - 1):
        ...
        if start_giro is not None and end_giro is not None:
            ...
            fixations_filtrate = FixationsDF[(FixationsDF['start timestamp [ns]'].astype('int64') >= start_giro) &
                                              (FixationsDF['start timestamp [ns]'].astype('int64') <= end_giro)]
            blinks_filtrati = BlinksDF[(BlinksDF['start timestamp [ns]'].astype('int64') >= start_giro) &
                                        (BlinksDF['start timestamp [ns]'].astype('int64') <= end_giro)]
            saccades_filtrati = SaccadesDF[(SaccadesDF['start timestamp [ns]'].astype('int64') >= start_giro) &
                                            (SaccadesDF['start timestamp [ns]'].astype('int64') <= end_giro)]
            pupille_filtrate = EyeStatesDF[(EyeStatesDF['timestamp [ns]'].astype('int64') >= start_giro) &
                                            (EyeStatesDF['timestamp [ns]'].astype('int64') <= end_giro)]

            # Calcola il tempo di giro
            tempo_giro = (end_giro - start_giro) / 1e9

            # Calcola la velocità media per giro
            velocita_media_giro = CalcolaVelocitaMediaGiro(fixations_filtrate)

            # Calcola il numero e la durata media delle fissazioni per il giro corrente
            num_fissazioni, durata_media_fissazioni = CalcolaFissazioni(fixations_filtrate)

            # Calcola il numero e la durata media dei blink per il giro corrente
            num_blinks, media_durata_blinks = CalcolaBlinks(blinks_filtrati)

            # Calcola il numero e la durata media delle saccadi per il giro corrente
            num_saccadi, media_durata_saccadi = CalcolaSaccadi(saccades_filtrati)

            # Calcola il diametro medio delle pupille per il giro corrente
            diametro_medio_pupille = CalcolaDiametroMedioPupille(pupille_filtrate)

            # Aggiungi i risultati al DataFrame finale
            final_df[current_giro] = [tempo_giro, velocita_media_giro, num_fissazioni, durata_media_fissazioni,
                                       num_blinks, media_durata_blinks, num_saccadi, media_durata_saccadi,
                                       diametro_medio_pupille]

    return final_df
"""

# Codice Python per la funzione StatisticheCurva1
code_statistiche_curva1 = """
def StatisticheCurva1(EventsDF, FixationsDF, BlinksDF, SaccadesDF, EyeStatesDF, cartella):
    \"\"\"
    Calcola le statistiche relative alla curva 1.
    ...
    Returns:
        pd.DataFrame: DataFrame contenente le statistiche relative alla curva 1.
    \"\"\"
    # Inizializza il DataFrame finale e la lista delle variabili
    final_df = pd.DataFrame(columns=['ID: ' + cartella])
...
# Codice Python per la funzione StatisticheCurva9
code_statistiche_curva9 = """
def StatisticheCurva9(EventsDF, FixationsDF, BlinksDF, SaccadesDF, EyeStatesDF, cartella):
    \"\"\"
    Calcola le statistiche relative alla curva 9.
    ...
    Returns:
        pd.DataFrame: DataFrame contenente le statistiche relative alla curva 9.
    \"\"\"
     # Inizializza il DataFrame finale e la lista delle variabili
    final_df = pd.DataFrame(columns=['ID: ' + cartella])
...
# Codice Python per la funzione StatisticheSettore1
code_statistiche_settore1 = """
def StatisticheSettore1(EventsDF, FixationsDF, BlinksDF, SaccadesDF, EyeStatesDF, cartella):
    \"\"\"
    Calcola le statistiche relative al settore 1.
    ...
    Returns:
        pd.DataFrame: DataFrame contenente le statistiche relative al settore 1.
    \"\"\"
    # Inizializza il DataFrame finale e la lista delle variabili
    final_df = pd.DataFrame(columns=['ID: ' + cartella])
...
#Soprascritte ci sono le importazioni di librerie e le variabili globali

#Qui vanno le altre funzioni nel seguente ordine : StatisticheGiri,StatisticheCurva1, StatisticheCurva9, StatisticheSettore1, CalcoloTotale, Principal

#Qui sotto la chiamata alla funzione Principal che esegue il codice

Principal()
"""

# Directory di destinazione per le immagini
image_directory = "/home/kenobi/Documenti/GitHub/Progetto-Ricerca/Code-Image"
...
def save_highlighted_image(code, filename):
    highlighted_code = highlight(code, PythonLexer(), ImageFormatter(
        font_name='arial', font_size=12, line_numbers=False, image_pad=20, style='default'))
    highlighted_image = Image.open(io.BytesIO(highlighted_code))
    save_path = os.path.join(image_directory, filename)
    highlighted_image.save(save_path)
    print(f"Immagine salvata: {save_path}")

# Salvataggio delle immagini per SalvaToExcel, StatisticheGiri, StatisticheCurva1, StatisticheCurva9 e StatisticheSettore1
save_highlighted_image(code_salva_to_excel, "SalvaToExcel.png")
save_highlighted_image(code_statistiche_giri, "StatisticheGiri.png")
save_highlighted_image(code_statistiche_curva1, "StatisticheCurva1.png")
save_highlighted_image(code_statistiche_curva9, "StatisticheCurva9.png")
save_highlighted_image(code_statistiche_settore1, "StatisticheSettore1.png")

# Salvataggio delle immagini per CalcoloTotale, CodePrincipal e CodeGlobale
save_highlighted_image(codeCalcoloTotale, "CalcoloTotale.png")
save_highlighted_image(CodePrincipal, "CodePrincipal.png")
save_highlighted_image(CodeGlobale, "CodeGlobale.png")