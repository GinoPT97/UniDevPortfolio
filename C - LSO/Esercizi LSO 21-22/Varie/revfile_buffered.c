...
int main(int argc, char *argv[]) {
	...
	len = safe_lseek(fd, -1, SEEK_END); //Ottengo la lunghezza del testo scritto nel file aperto, portando il puntatore del testo alla fine del file, escludendo il carattere EOF
	...
	do {
		...
		memset(buffer, 0, sizeof(buffer)); //Pulisco il buffer per evitare i problemi che possono essere causati da una stringa sporca
