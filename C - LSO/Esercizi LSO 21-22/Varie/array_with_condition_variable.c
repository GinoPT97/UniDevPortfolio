...
typedef struct thread_arg {
  ...
  pthread_cond_t something_produced;
  pthread_cond_t something_consumed;
} * thread_arg;
...
int main(int argc, char *argv[]) {
  ...
  int err_init;
  if ((err_init = pthread_mutex_init(&arg.arg_mutex, NULL))) {
    printf("Error init struct arg mutex: %s\n", strerror(err_init));
    exit(EXIT_FAILURE);
  }
  if ((err_init = pthread_cond_init(&arg.something_produced, NULL))) {
    printf("Error init struct arg condtion variable (produced): %s\n",
           strerror(err_init));
    exit(EXIT_FAILURE);
  }
  if ((err_init = pthread_cond_init(&arg.something_consumed, NULL))) {
    printf("Error init struct arg condtion variable (consume): %s\n",
           strerror(err_init));
    exit(EXIT_FAILURE);
  }
  ...
  while (arg.num_item > 0) {
  }
  pthread_cond_broadcast(&arg.something_produced);
  for (int i = 0; i < m; i++) {
...
void *produce_job(void *param) {
  ...
  int i = 0;
  while (i <= rand_iter) {
    if (pthread_mutex_lock(&arg->arg_mutex) == 0) {
      if (arg->num_item == ARRAY_SIZE) {
        pthread_cond_wait(&arg->something_consumed, &arg->arg_mutex);
      }
      arg->array[arg->cur_pos_write] = rand();
      arg->cur_pos_write = (arg->cur_pos_write + 1) % ARRAY_SIZE;
      arg->num_item++;
      i++;
      pthread_cond_signal(&arg->something_produced);
      pthread_mutex_unlock(&arg->arg_mutex);
    } else { /* EBUSY*/
    }
  }
...
void *consume_job(void *param) {
  ...
  while (1) {
    if (pthread_mutex_lock(&arg->arg_mutex) == 0) {
      if (arg->num_item == 0) {
        pthread_cond_wait(&arg->something_produced, &arg->arg_mutex);
      }
      if (arg->num_item == 0) {
        pthread_mutex_unlock(&arg->arg_mutex);
        return NULL;
      ...
      arg->num_item--;
      pthread_cond_signal(&arg->something_consumed);
