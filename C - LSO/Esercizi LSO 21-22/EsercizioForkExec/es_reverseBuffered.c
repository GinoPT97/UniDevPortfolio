...
int main(int argc, char **argv)
{
	...
	{printf("Dimensione buffer di %d non valida (dimensione massima supportata %ld)\n", atoi(argv[2]), infoFile.st_size), exit(EXIT_FAILURE);}	
	...
	if(currentOffset < 0){perror("Errore seeking (1)"), exit(EXIT_FAILURE);}	// controllo lseek

	bytesToRead = infoFile.st_size;	// Quantità di bytes totali da leggere dal file (inizialmente uguale alla dimensione del file)
	count = BUF_SIZE; 		// Quantità di bytes da leggere/scrivere ad ogni read/write (inizialmente uguale alla dimensione del buffer - 1)
	...
	while(bytesToRead > 0) // Finché ci sono bytes da leggere
	{
		...
		if(currentOffset < 0){perror("Errore seeking (2)"), exit(EXIT_FAILURE);} // controllo lseek
		...
		if(bytesWrite != count){perror("Errore scrittura file (1)"), exit(EXIT_FAILURE);} // controllo write

		currentOffset = lseek(fd, -count, SEEK_CUR); // Modifica offset: -[count] dalla posizione corrente
		if(currentOffset < 0){perror("Errore seeking (3)"), exit(EXIT_FAILURE);} // controllo lseek
