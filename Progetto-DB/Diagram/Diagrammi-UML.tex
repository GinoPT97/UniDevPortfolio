...
% Specializzazioni di Prodotto - disposte in griglia più ampia
\node[class] (frutta) at (9,-12) {
  \textbf{Frutta}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + dataraccolta : date
};

\node[class] (verdura) at (12,-12) {
  \textbf{Verdura}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + dataraccolta : date
};

\node[class] (latticini) at (15,-12) {
...
\node[class] (farinacei) at (9,-15) {
  \textbf{Farinacei}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + glutine : boolean
};

\node[class] (uova) at (12,-15) {
  \textbf{Uova}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + datascadenza : date
};

\node[class] (confezionati) at (15,-15) {
...
\draw[relation] (articoli2) .. controls (5,-9.5) and (8.5,-6) .. node[midway, below] {riferisce (N:1)} (prodotto2);
