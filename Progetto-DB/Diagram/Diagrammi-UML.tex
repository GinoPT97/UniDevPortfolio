...
% Specializzazioni di Prodotto - disposte con ancora più spazio orizzontale
\node[class] (frutta) at (4,-12) {
  \textbf{Frutta}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + dataraccolta : date
};

\node[class] (verdura) at (8,-12) {
  \textbf{Verdura}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + dataraccolta : date
};

\node[class] (latticini) at (12,-12) {
...
\node[class] (farinacei) at (4,-15) {
  \textbf{Farinacei}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + glutine : boolean
};

\node[class] (uova) at (8,-15) {
  \textbf{Uova}\\[-0.3em]\rule{\linewidth}{0.2pt}\\
  + datascadenza : date
};

\node[class] (confezionati) at (12,-15) {
...
\draw[relation] (ordine2) -- node[midway, left] {contiene (1:N)} (articoli2);
    \draw[relation] (articoli2) .. controls (12,-9.5) and (16,-6) .. node[midway, below] {riferisce (N:1)} (prodotto2);
