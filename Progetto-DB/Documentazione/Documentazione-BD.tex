...
\begin{itemize}
    \item La \textbf{classe Prodotto} è stata unificata e arricchita con un attributo enumerativo \texttt{categoria}, che permette di distinguere le diverse tipologie di articoli. Gli attributi specifici (es. \emph{data raccolta}, \emph{mungitura}, \emph{scadenza}, \emph{glutine}) sono ora integrati nella stessa classe e possono assumere valore nullo quando non pertinenti;
    \item Le relazioni tra \textbf{Cliente}, \textbf{Tessera}, \textbf{Ordine} e \textbf{ArticoliOrdine} sono mantenute inalterate, poiché già rispecchiano correttamente i vincoli cardinali e semantici del dominio;
    \item Sono state introdotte \textbf{enumerazioni} (come \texttt{STATOTESSERA} e \texttt{TIPOLOGIA}) per rappresentare domini discreti e garantire coerenza dei valori nei dati.
\end{itemize}
