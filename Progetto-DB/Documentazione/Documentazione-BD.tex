...
\arrayrulecolor{myblue}
{\small
\begin{tabular}{|p{2.6cm}|p{3cm}|p{2.6cm}|p{5.2cm}|}
\hline
...
\hline
\rowcolor{mygray} \texttt{\detokenize{TrgAggiorna}\newline\detokenize{ScortaPunti}} &
AFTER INSERT ON \texttt{articoli\-ordine} &
\texttt{\detokenize{Aggiorna}\newline\detokenize{ScortaEPunti()}} &
Aggiorna automaticamente la scorta dei prodotti acquistati e i punti accumulati dal cliente. \\
\hline
\texttt{\detokenize{TrgAggiorna}\newline\detokenize{PrezzoOrdine}} &
AFTER INSERT ON \texttt{articoli\-ordine} &
\texttt{\detokenize{Aggiorna}\newline\detokenize{PrezzoOrdine()}} &
Calcola e aggiorna il prezzo totale dell'ordine dopo l'inserimento di nuovi articoli. \\
\hline
\rowcolor{mygray} \texttt{\detokenize{TrgVerifica}\newline\detokenize{ScortaProdotto}} &
BEFORE INSERT ON \texttt{articoli\-ordine} &
\texttt{\detokenize{Verifica}\newline\detokenize{ScortaProdotto()}} &
Blocca l'inserimento se la quantità richiesta supera la disponibilità di magazzino. \\
\hline
\texttt{\detokenize{TrgAggiorna}\newline\detokenize{StatoTessera}} &
BEFORE INSERT OR UPDATE ON \texttt{tessera} &
\texttt{\detokenize{Aggiorna}\newline\detokenize{StatoTessera()}} &
Aggiorna lo stato della tessera fedeltà in base ai punti o alla data di scadenza. \\
\hline
\rowcolor{mygray} \texttt{\detokenize{TrgRipristina}\newline\detokenize{ScortaPunti}} &
AFTER DELETE ON \texttt{articoli\-ordine} &
\texttt{\detokenize{Ripristina}\newline\detokenize{ScortaProdotto()}} &
Ripristina scorte e punti del cliente dopo la cancellazione di un articolo d'ordine. \\
\hline
\texttt{\detokenize{TrgAggiorna}\newline\detokenize{PrezzoOrdine}\newline\detokenize{Delete}} &
AFTER DELETE ON \texttt{articoli\-ordine} &
\texttt{\detokenize{Aggiorna}\newline\detokenize{PrezzoOrdine}\newline\detokenize{Delete()}} &
