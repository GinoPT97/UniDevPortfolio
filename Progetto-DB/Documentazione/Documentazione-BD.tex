...
\setlength{\emergencystretch}{3em}  % Previene Overfull hbox
\setlength{\parindent}{0pt}         % Rimuove indentazione paragrafi
\setlength{\parskip}{0.5em}         % Aggiunge spazio tra paragrafi
\setlength{\textwidth}{16cm}        % Aumenta larghezza testo
\setlength{\textheight}{24cm}       % Aumenta altezza testo
\tolerance=1000                      % Permette box più flessibili
\pretolerance=100                   % Migliora la sillabazione
\setlength{\columnseprule}{0.4pt}   % Linea tra colonne se necessario         
...
\renewcommand{\arraystretch}{1.2} % se usi colori come myblue/mygray
...
\begin{itemize}
    ...
    \item Vincoli \texttt{CHECK} su valori numerici (\texttt{prezzo} $\geq$ \texttt{0}, \texttt{scorta} $\geq$ \texttt{0}), validazione di email e codice fiscale tramite regex.
...
Ricalcola il prezzo totale dell'ordine sommando il costo dei nuovi articoli (\texttt{prezzo} $\times$ \texttt{numeroarticoli}). \\
\hline
\rowcolor{mygray}
...
Ricalcola il prezzo totale sottraendo il valore degli articoli rimossi (risultato $\geq 0$). \\
\hline
\end{tabularx}
}
\arrayrulecolor{black}
...
Aggiorna il campo \texttt{prezzototale} sommando il valore dei nuovi articoli (\texttt{prezzo} $\times$ \texttt{numeroarticoli}).
...
\newpage
\subsection{Schema Logico Dettagliato}

\subsection{Trigger e Procedure}
\begin{tcolorbox}[enhanced,breakable,colback=mygray,colframe=myblue,title=Panoramica Trigger]
\begin{itemize}
    \item \textbf{TrgAggiornaScortaPunti}
        \begin{itemize}
            \item Evento: AFTER INSERT su articoliordine
            \item Funzione: AggiornaScortaEPunti()
            \item Scopo: Gestione atomica di scorte e punti fedeltà
            \item Esempio: Acquisto di 3 mele a 1.20€:
                \begin{itemize}
                    \item Scorta mele: 50 → 47
                    \item Punti cliente: +0.36 (10\% di 3.60€)
                \end{itemize}
        \end{itemize}
    
    \item \textbf{TrgAggiornaPrezzoOrdine}
        \begin{itemize}
            \item Evento: AFTER INSERT su articoliordine
            \item Funzione: AggiornaPrezzoOrdine()
            \item Scopo: Calcolo automatico del totale ordine
            \item Esempio: Aggiunta 3 mele a 1.20€:
                \begin{itemize}
                    \item Totale ordine: +3.60€
                \end{itemize}
        \end{itemize}
    
    \item \textbf{TrgVerificaScortaProdotto}
        \begin{itemize}
            \item Evento: BEFORE INSERT su articoliordine
            \item Funzione: VerificaScortaProdotto()
            \item Scopo: Prevenzione vendite con scorta insufficiente
            \item Esempio: Tentativo acquisto 5 mele con scorta 3:
                \begin{itemize}
                    \item Risultato: Errore "Scorta insufficiente"
                \end{itemize}
        \end{itemize}
        
    \item \textbf{TrgAggiornaStatoTessera}
        \begin{itemize}
            \item Evento: BEFORE INSERT OR UPDATE su tessera
            \item Funzione: AggiornaStatoTessera()
            \item Scopo: Gestione automatica scadenza tessere
            \item Esempio: Tessera con data scadenza superata:
                \begin{itemize}
                    \item Stato: ATTIVA → SCADUTA
                \end{itemize}
        \end{itemize}
\end{itemize}
\end{tcolorbox}

\subsection{Flusso di Esecuzione dei Trigger}
\begin{tcolorbox}[enhanced,breakable,colback=mygray,colframe=myblue,title=Sequenza Operazioni]
Esempio di inserimento nuovo articolo in un ordine:

\begin{enumerate}
    \item \textbf{TrgVerificaScortaProdotto}
        \begin{itemize}
            \item Verifica disponibilità prodotto
            \item Se insufficiente: blocca operazione
        \end{itemize}
    
    \item \textbf{TrgAggiornaScortaPunti}
        \begin{itemize}
            \item Decrementa scorta prodotto
            \item Calcola punti (10\% del valore)
            \item Aggiorna saldo tessera cliente
        \end{itemize}
    
    \item \textbf{TrgAggiornaPrezzoOrdine}
        \begin{itemize}
            \item Calcola nuovo importo articoli
            \item Aggiorna totale ordine
        \end{itemize}
\end{enumerate}

\textbf{Gestione Errori:}
\begin{itemize}
    \item Rollback automatico in caso di errori
    \item Ripristino stato precedente
    \item Messaggi di errore specifici
\end{itemize}
\end{tcolorbox}

\subsection{Funzioni di Supporto}
\begin{tcolorbox}[enhanced,breakable,colback=mygray,colframe=myblue,title=Funzioni Principali]
\begin{itemize}
    \item \textbf{RicercaClientiPerCategoria}
        \begin{itemize}
            \item Input: categoria (TIPOLOGIA)
            \item Output: clienti con punti/spese per categoria
            \item Esempio: 
                \begin{verbatim}
SELECT * FROM RicercaClientiPerCategoria('FRUTTA')
WHERE punti_categoria > 50;
                \end{verbatim}
        \end{itemize}
    
            \item \textbf{DipendenteStatistiche}
                \begin{itemize}
                    \item Input: \texttt{data\_inizio}, \texttt{data\_fine}
                    \item Output: performance dipendenti
                    \item Esempio:
                        \begin{verbatim}
SELECT * FROM DipendenteStatistiche(
