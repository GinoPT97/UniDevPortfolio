...
Il presente progetto riguarda lo sviluppo di un sistema informativo per un negozio di ortofrutta, composto da una base di dati relazionale e un'applicazione Java con interfaccia grafica (GUI) realizzata con Swing o JavaFX.  
L'obiettivo del sistema è supportare le attività quotidiane del negozio, facilitando la gestione delle vendite, del magazzino e dei clienti, e fornendo strumenti per analizzare le performance di vendita.

Il sistema deve consentire di:
\begin{itemize}
    \item Gestire i prodotti disponibili, classificati per tipologia (frutta, verdura, farinacei, latticini, uova e prodotti confezionati), ciascuna con caratteristiche specifiche (ad esempio, la frutta e la verdura hanno una data di raccolta, i latticini hanno una data di mungitura e di produzione, ecc.);
    \item Registrare i clienti e gestire le tessere punti, calcolando automaticamente i punti fedeltà (10\% del valore della spesa) per ogni acquisto;
    \item Ricercare i clienti in base ai prodotti acquistati e alla quantità di punti accumulati per ogni categoria;
    \item Gestire i dipendenti associando ciascuna vendita al personale responsabile, con possibilità di analizzare il numero di vendite effettuate e l’introito generato in un determinato periodo.
\end{itemize}

Il progetto si inserisce in un contesto accademico volto a sviluppare competenze nella progettazione di database relazionali, nello sviluppo di applicazioni Java con GUI e nell’implementazione di sistemi informativi completi.
...
\begin{itemize}
    \item La gestione dei prodotti per tipologia (frutta, verdura, farinacei, latticini, uova e prodotti confezionati), con caratteristiche specifiche per ciascuna categoria;
    \item La gestione dei clienti, con registrazione delle informazioni personali e delle tessere punti;
    \item La gestione dei dipendenti, con associazione di ciascuna vendita al personale responsabile;
    \item La generazione di statistiche e report, come l’analisi dei clienti per categorie di prodotti acquistati, dei punti fedeltà accumulati e delle performance dei dipendenti;
    \item La protezione dei dati sensibili dei clienti e l’affidabilità delle operazioni quotidiane del negozio.
\end{itemize}
...
Il diagramma Entità-Relazioni rappresenta la struttura logica dei dati e le interconnessioni tra le entità principali del sistema.  
Nel modello sono state individuate le seguenti entità principali:

\begin{itemize}
  \item \textbf{CLIENTE}: rappresenta l’utente che effettua acquisti. È identificato dal codice cliente e possiede attributi anagrafici e di contatto come nome, cognome, codice fiscale, indirizzo, telefono ed e-mail.
  \item \textbf{TESSERA}: associata univocamente a un cliente, contiene informazioni sul numero di punti accumulati, la data di emissione, la data di scadenza e lo stato (attiva, sospesa o scaduta).
  \item \textbf{ORDINE}: rappresenta un acquisto effettuato da un cliente e gestito da un dipendente. Comprende data d’acquisto, prezzo totale e codice identificativo.
  \item \textbf{DIPENDENTE}: rappresenta il personale che gestisce gli ordini. Possiede dati anagrafici simili a quelli del cliente e un proprio codice identificativo.
  \item \textbf{PRODOTTO}: rappresenta gli articoli venduti. È specializzato in varie categorie (\textit{Frutta}, \textit{Verdura}, \textit{Latticini}, \textit{Farinacei}, \textit{Uova}, \textit{Confezionati}) con attributi specifici per ciascuna.
  \item \textbf{ARTICOLI\_ORDINE}: entità debole che rappresenta la relazione tra un ordine e i prodotti acquistati, contenente il prezzo e il numero di articoli.
\end{itemize}

Le principali relazioni tra le entità sono:
\begin{itemize}
  \item \textbf{possiede}: collega un cliente alla propria tessera.
  \item \textbf{effettua}: associa un cliente agli ordini che ha effettuato.
  \item \textbf{gestisce}: collega un dipendente agli ordini che ha gestito.
  \item \textbf{contiene}: associa un ordine ai prodotti acquistati (tramite l’entità debole \textit{ARTICOLI\_ORDINE}).
\end{itemize}
...
Il diagramma concettuale traduce il modello ER in un formato più astratto, basato su classi e relazioni, che evidenzia l’organizzazione semantica dei dati.  
Le entità vengono rappresentate come classi con attributi e relazioni (associazioni, composizioni, generalizzazioni).

\begin{itemize}
  \item \textbf{Persona} è una superclasse astratta che generalizza le caratteristiche comuni di \textit{Cliente} e \textit{Dipendente}.
  \item \textbf{Cliente} e \textbf{Dipendente} ereditano gli attributi della superclasse e aggiungono un proprio identificatore.
  \item \textbf{Ordine} rappresenta la transazione commerciale e funge da punto di connessione tra cliente, dipendente e articoli ordinati.
  \item \textbf{ArticoliOrdine} funge da classe di associazione tra \textit{Ordine} e \textit{Prodotto}, contenendo il prezzo e la quantità acquistata.
  \item \textbf{Prodotto} è una classe generalizzata con specializzazioni che descrivono categorie merceologiche distinte, ognuna con attributi specifici (es. data di raccolta, presenza di glutine, data di scadenza, ecc.).
\end{itemize}
...
Il diagramma concettuale ristrutturato è il risultato della normalizzazione del modello precedente.  
Le specializzazioni di \textit{Prodotto} sono state accorpate in un’unica classe che include un attributo enumerativo \textit{categoria}, mentre gli attributi specifici delle varie tipologie di prodotti sono stati integrati direttamente nella stessa classe (eventualmente lasciati nulli quando non pertinenti).

\begin{itemize}
  \item La classe \textbf{Prodotto} diventa quindi più completa, contenendo tutti i possibili attributi (data raccolta, mungitura, produzione, glutine, scadenza, ecc.).
  \item La classe \textbf{Tessera} mantiene un’associazione di composizione con il cliente, mentre \textbf{Ordine} e \textbf{ArticoliOrdine} restano strutturati come nella fase precedente.
  \item Sono state introdotte enumerazioni per rappresentare in modo controllato i valori discreti, come lo stato della tessera (\textit{ATTIVA, SOSPESA, SCADUTA}) e la tipologia di prodotto.
\end{itemize}
...
\begin{tabularx}{\textwidth}{|
    >{\ttfamily\small\raggedright\arraybackslash}X |
    >{\centering\arraybackslash}p{3cm} |
    >{\ttfamily\small\raggedright\arraybackslash}X |
    >{\raggedright\arraybackslash}X |}
\hline
\rowcolor{myblue}\textcolor{white}{\textbf{Trigger}} &
\textcolor{white}{\textbf{Evento}} &
\textcolor{white}{\textbf{Funzione Richiamata}} &
\textcolor{white}{\textbf{Scopo / Descrizione}} \\
\hline
\rowcolor{mygray} \texttt{\detokenize{TrgAggiornaScortaPunti}} &
AFTER INSERT ON \texttt{articoliordine} &
\texttt{\detokenize{AggiornaScortaEPunti()}} &
Aggiorna automaticamente la scorta dei prodotti acquistati e i punti accumulati dal cliente. \\
\texttt{\detokenize{TrgAggiornaPrezzoOrdine}} &
AFTER INSERT ON \texttt{articoliordine} &
\texttt{\detokenize{AggiornaPrezzoOrdine()}} &
Calcola e aggiorna il prezzo totale dell’ordine dopo l’inserimento di nuovi articoli. \\
\rowcolor{mygray} \texttt{\detokenize{TrgVerificaScortaProdotto}} &
BEFORE INSERT ON \texttt{articoliordine} &
\texttt{\detokenize{VerificaScortaProdotto()}} &
Blocca l’inserimento se la quantità richiesta supera la disponibilità di magazzino. \\
\texttt{\detokenize{TrgAggiornaStatoTessera}} &
BEFORE INSERT OR UPDATE ON \texttt{tessera} &
\texttt{\detokenize{AggiornaStatoTessera()}} &
Aggiorna lo stato della tessera fedeltà in base ai punti o alla data di scadenza. \\
\rowcolor{mygray} \texttt{\detokenize{TrgRipristinaScortaPunti}} &
AFTER DELETE ON \texttt{articoliordine} &
\texttt{\detokenize{RipristinaScortaProdotto()}} &
Ripristina scorte e punti del cliente dopo la cancellazione di un articolo d’ordine. \\
\texttt{\detokenize{TrgAggiornaPrezzoOrdineDelete}} &
AFTER DELETE ON \texttt{articoliordine} &
\texttt{\detokenize{AggiornaPrezzoOrdineDelete()}} &
Aggiorna il prezzo totale dell’ordine dopo la rimozione di articoli. \\
\hline
\end{tabularx}
\caption{Mappatura logica tra trigger, eventi e funzioni PL/pgSQL.}
\end{table}
\arrayrulecolor{black}
