...
Il presente progetto riguarda lo sviluppo di un sistema informativo per un negozio di ortofrutta, finalizzato a supportare le attività quotidiane del punto vendita.  
L’obiettivo principale del sistema è facilitare la gestione delle vendite, del magazzino e dei clienti, fornendo al tempo stesso strumenti per analizzare le performance e ottimizzare le operazioni interne.

Il sistema è progettato per consentire di:
\begin{itemize}
    \item Gestire i prodotti disponibili, classificati per tipologia (frutta, verdura, farinacei, latticini, uova e prodotti confezionati), ciascuna caratterizzata da attributi specifici (ad esempio, la frutta e la verdura hanno una data di raccolta, mentre i latticini presentano una data di mungitura e una data di produzione);
...
Il progetto si inserisce in un contesto accademico volto a sviluppare competenze nella progettazione di sistemi informativi e nella gestione di basi di dati relazionali, con particolare attenzione agli aspetti di modellazione dei dati, efficienza delle operazioni e sicurezza delle informazioni.
...
Il progetto prevede lo sviluppo di un sistema informativo per la gestione di un negozio di ortofrutta.  
Il sistema deve consentire di gestire le vendite e la disponibilità dei prodotti, identificando le diverse tipologie merceologiche (frutta, verdura, farinacei, latticini, uova e confezionati). Ogni categoria presenta caratteristiche specifiche: ad esempio, la frutta e la verdura dispongono di una data di raccolta, mentre i latticini riportano sia la data di mungitura del latte sia la data di produzione.

Ogni cliente deve essere registrato nel sistema e disporre di una tessera punti che lo identifica presso il negozio. Per ogni acquisto, il cliente riceve un numero di punti fedeltà pari al 10\% del valore della spesa effettuata.  
Il sistema deve inoltre consentire di ricercare i clienti, distinguendoli in base alle categorie di prodotti acquistati e al numero di punti accumulati per ciascuna tipologia di prodotto.

Nel caso in cui siano presenti più dipendenti, ogni vendita deve essere associata al dipendente che l’ha effettuata. Deve pertanto essere possibile individuare, in un determinato periodo, il dipendente che ha realizzato il maggior numero di vendite e quello che ha generato il maggior introito per il negozio.
